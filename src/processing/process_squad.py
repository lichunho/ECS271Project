"""Process SQuAD DPR-curated JSON into unified per-instance JSONL.

Port of: https://github.com/starsuzi/Adaptive-RAG/blob/main/processing_scripts/process_squad.py
...
def main(input_dir: Path, output_dir: Path) -> dict[str, int]:
    """Process SQuAD train + dev into ``{output_dir}/{train,dev}.jsonl``.

    Expects ``input_dir`` to contain ``biencoder-squad1-{train,dev}.json``.
    """
    ...
    train_in = input_dir / "biencoder-squad1-train.json"
    train_out = output_dir / "train.jsonl"
    counts["train"] = write_dpr_instances_to_filepath(
        read_json(train_in), train_out, "train",
        dataset_label="squad", qid_prefix="single_squad_",
    )

    dev_in = input_dir / "biencoder-squad1-dev.json"
